...
"""
This is where the parameters are defined and where the code is started.

We first create a flag object that contains all the settings of the model that we create.
...
# get the current directory and create the other paths from there:
home = os.getcwd()
train_file = "../datasets/squad/train-v2.0.json" #os.path.join(home, "datasets", "squad", "train-v1.1.json")
dev_file = "../datasets/squad/dev-v2.0.json" #os.path.join(home, "datasets", "squad", "dev-v1.1.json")
test_file = "../datasets/squad/dev-v2.0.json" #os.path.join(home, "datasets", "squad", "dev-v1.1.json")
glove_word_file = "../datasets/glove/glove.840B.300d.txt" #os.path.join(home, "datasets", "glove", "glove.840B.300d.txt")

train_dir = "train"
load_model_name = "QANetO"
model_name = "EQUANeT3"
dir_name = os.path.join(train_dir, model_name)
load_dir_name = os.path.join(train_dir, load_model_name)
if not os.path.exists(load_dir_name):
    EnvironmentError
...
load_dir = os.path.join(load_dir_name, "model")
...
# define the action the will be performed:
# one of: train, test, debug, prepro
mode = "test"
flags.DEFINE_string("mode", mode, "Running mode train/debug/test")

# be sure we ahve the correct word embedding
use_squad1_dict = True
flags.DEFINE_bool("use_squad1_dict", use_squad1_dict, "Corpus size for Glove")
if use_squad1_dict:
    word_emb_file = "data/SQuAD1/word_emb.json"
    char_emb_file = "data/SQuAD1/char_emb.json"
    word_dictionary = "data/SQuAD1/word_dictionary.json"
    char_dictionary = "data/SQuAD1/char_dictionary.json"
else:
...
flags.DEFINE_string("save_dir", save_dir, "Directory from which we load model")
flags.DEFINE_string("load_dir", load_dir, "Directory for saving model")
...
# possibility to add downsampling during the preprocessing in order to create a smaller dataset
flags.DEFINE_float("portion_downsampling", 0.9, "Portion of discarted data during the preprocessing")

# specify the number of batch used to train
flags.DEFINE_integer("num_steps", 60003, "Number of steps")     # 60000
